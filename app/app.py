...
def index():
  if request.method == "GET":
    ...
    return render_template("index.html", comments=comments, replies=replies, likes=likes)
...
def register():
  if request.method == "GET":
    return render_template("register_page.html", error_one=False, error_two=False, error_three=False)

  username = request.form["username"]
  password = request.form["password"]

  if User.query.filter_by(username=username).first():
    return render_template("register_page.html", error_one=True, error_two=False, error_three=False)

  if username == '':
    return render_template("register_page.html", error_one=False, error_two=True, error_three=False)

  if password == '':
    return render_template("register_page.html", error_one=False, error_two=False, error_three=True)
